...
P.innerHTML = 'Show Labels ';
const select = document.createElement('select');
select.setAttribute('id', 'label-option');
const OOne = document.createElement('option');
OOne.setAttribute('value', 'all');
OOne.innerHTML = 'All';
const OTwo = document.createElement('option');
OTwo.setAttribute('value', 'minmax');
OTwo.innerHTML = 'Min/Max';
const OThree = document.createElement('option');
OThree.setAttribute('value', 'none');
OThree.innerHTML = 'None';
...
d3.select('select')
  .on('change', (d) => {
    // Store the value of the option selected
    const sel = d3.select('#label-option').node().value;

    svg.selectAll('text')
      .data(dataList)
      .text((d) => showMinMax(dataList, 'sales', d.sales, sel))
  });
